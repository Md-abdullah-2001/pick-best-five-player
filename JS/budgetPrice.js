...
document
  ...
  .addEventListener("click", function () {
    ...
    if (
      isNaN(managerField.value) ||
      isNaN(coachField.value) ||
      ManagerFieldValue < 0 ||
      coachFieldValue < 0 ||
      ...
      alert(
        "Input error : input should be in digits,empty field not allowed, and not should be a negetive number!"
      );
      managerField.value = "";
      coachField.value = "";
    ...
    // sum of total cost
    const totalExpenses =
      ManagerFieldValue + coachFieldValue + playerExpensesInnerText;
